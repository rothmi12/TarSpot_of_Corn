...
#Separate out healthy and diseased for plotting, since they have different # of reads
pooled_pruned_healthy <- pooled_pruned
pooled_pruned_healthy_OTU <- pooled_pruned_healthy@otu_table[,45:47]
pooled_pruned_healthy_TAX <- pooled_pruned_healthy@tax_table
pooled_pruned_healthy_SAM <- tarspot_meta[tarspot_meta$plant_status=="Healthy",]
healthy <- phyloseq(otu_table(pooled_pruned_healthy_OTU),
                    tax_table(pooled_pruned_healthy_TAX),
                    sample_data(pooled_pruned_healthy_SAM))

pooled_pruned_disease <- pooled_pruned
pooled_pruned_disease_OTU <- pooled_pruned_disease@otu_table[,1:44]
pooled_pruned_disease_TAX <- pooled_pruned_disease@tax_table
pooled_pruned_disease_SAM <- tarspot_meta[tarspot_meta$plant_status!="Healthy",]
disease <- phyloseq(otu_table(pooled_pruned_disease_OTU),
                    tax_table(pooled_pruned_disease_TAX),
                    sample_data(pooled_pruned_disease_SAM))


cl <- colors(distinct = TRUE)
set.seed(42)
mycols <- sample(cl,60)
p1 <- plot_bar(healthy, "plant_status", fill = "Genus") +
  xlab("Lesion Type") +
  theme(axis.title.x = element_text(face="bold", size = 20),
  ...
  geom_bar(aes(x = plant_status, y = Abundance, fill=Genus), stat="identity", position="stack") +
...
p2 <- plot_bar(disease, "plant_status", fill = "Genus") +
  xlab("Lesion Type") +
  theme(axis.title.x = element_text(face="bold", size = 20),
        axis.text.x = element_text(face="bold", size=15, angle = 270),
        axis.title.y = element_text(face="bold", size = 20),
...
##############################################################
for (i in 1:100){
  indicator.dist <- indicspecies::multipatt(as.data.frame(t(tarspot_physeq@otu_table)), cluster = tarspot_physeq@sam_data$plant_status, duleg = T)
  FE.ind <- data.frame(cbind(indicator.dist$sign[colnames(indicator.dist$sign)=="s.Fish eye"]),
                     indicator.dist$sign[colnames(indicator.dist$sign)=="p.value"])
  FE.ind$OTU <- rownames(FE.ind)
  for (j in seq_along(FE.ind$s.Fish.eye)){
  if (FE.ind$s.Fish.eye[j]==1 && FE.ind$p.value[j] < 0.05){
    FE.ind$sig[j] <- "significant"
  }
  else{
    FE.ind$sig[j] <- "no"
  }
  }
  if (sum(FE.ind$s.Fish.eye)==0){
    FE.ind.sig <- FE.ind[FE.ind$sig=="significant",]
  }
  else{
    FE.ind.sig <- rbind(FE.ind.sig, FE.ind[FE.ind$sig=="significant",])
  }
FE.ind <- NULL
print(i)
}
write.table(FE.ind.sig, "FE_ind_sig_full.txt", sep = "\t")
```
I got 9 OTUs that were significant
OTU_1, 100/100, average p = 0.02065
OTU_2, 100/100, average p = 0.00515
OTU_6, 96/100, average p = 0.025052
OTU_7, 97/100, average p = 0.018505
OTU_18, 98/100, average p = 0.02260
OTU_33, 100/100, average p = 0.00515
OTU_207, 59/100, average p = 0.03593
OTU_262, 100/100, average p = 0.0057
OTU_466, 20/100, average p = 0.03675
...
write.table(FE.ind.sig, "FE_ind_sig_pruned.txt", sep = "\t")
```

I got 7 OTUs that were significant
OTU_1, 93/100, average p = 0.02962
OTU_2, 100/100, average p = 0.00515
OTU_6, 28/100, average p = 0.03643
OTU_18, 52/100, average p = 0.03596
OTU_33, 100/100, average p = 0.00505
OTU_207, 41/100, average p = 0.03805
OTU_262, 100/100, average p = 0.00765

So, the only 2 missing from the full data set are OTU_7 and OTU_466

Let's see what all of these species actually are.

```{r}
rare.otu <- as.data.frame(pooled_pruned@tax_table)
rare.otu$otu <- rownames(rare.otu)
indicator.tax <- rare.otu[rare.otu$otu == "OTU_1",]
indicator.tax[2,] <- rare.otu[rare.otu$otu == "OTU_2",]
indicator.tax[3,] <- rare.otu[rare.otu$otu == "OTU_6",]
indicator.tax[4,] <- rare.otu[rare.otu$otu == "OTU_18",]
indicator.tax[5,] <- rare.otu[rare.otu$otu == "OTU_33",]
indicator.tax[6,] <- rare.otu[rare.otu$otu == "OTU_207",]
indicator.tax[7,] <- rare.otu[rare.otu$otu == "OTU_262",]
indicator.tax[8,] <- rare.otu[rare.otu$otu == "OTU_7",]
#indicator.tax[9,] <- rare.otu[rare.otu$otu == "OTU_466",]
